...
class PlatformForm(forms.Form):
    ...
    owner = forms.ModelChoiceField(
        queryset=Publisher.objects.all(), label="Owner", required=False
    )
    description = forms.CharField(
        max_length=5000,
        required=False,
...
class GameForm(forms.Form):
    ...
    publisher = forms.ModelChoiceField(
        queryset=Publisher.objects.all(), label="Publisher", required=False
    )
    platform = forms.ModelChoiceField(
        queryset=Platform.objects.all(), label="Platform", required=False
    )
    release_date = forms.IntegerField(required=False)
    description = forms.CharField(
        max_length=5000,
...
class ReviewForm(forms.ModelForm):
    class Meta:
        model = Review
        fields = ["rate", "review"]
        widget = {"rate": forms.Select(attrs={"class": "form-control"})}
